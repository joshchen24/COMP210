...
public class LinkedList<T> {
    ...
    public boolean isEqual(LinkedList list2) {
        Node<T> l2node = this.getHead();
        Node<T> l1node = list2.getHead();
        if (this.size != list2.size){
            return false;
        }
        for (int i = 0; i < this.size; i++){
            if (l2node.getValue() != l1node.getValue()){
                return false;
            }
    ...
    public void removeRepeats() {
        if (this.size() > 0) {
            Node<T> lagNode = this.getHead();
            Node<T> leadNode = this.getHead().getNext();
            for (int i = 0; i < this.size() - 1; i++) {
                if (leadNode != null) {
                    if (lagNode.getValue().equals(leadNode.getValue())) {
                        lagNode.setNext(leadNode.getNext());
                        leadNode = lagNode.getNext();
                        this.size--;
                    }
                    if (leadNode != null) {
    ...
    public void merge(LinkedList list2) {
        ...
        Node<T> node1Next = null;
        Node<T> node2 = list2.getHead();
        Node<T> node2Next = null;
        for (int i = 0; i < list2.size(); i++){
            node1Next = node1.getNext();
            node2Next = node2.getNext();
            node1.setNext(node2);
